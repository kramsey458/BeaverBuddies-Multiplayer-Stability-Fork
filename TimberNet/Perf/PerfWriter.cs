...
{
    /// <summary>
    /// Writes the log's rows to a file on a thread of its own, twice a second, so no disk work ever happens on the frame being
    /// measured. Rows are formatted into buffers allocated once. If the file cannot be opened or written the writer gives up quietly
    ...
    public sealed class PerfWriter
    {
        ...
        /// <summary>Called on the writer thread before each batch of rows is written, to add comment lines the rows depend on (names).</summary>
        public Action<TextWriter>? BeforeRows { get; set; }

        /// <summary>Called on the writer thread once, after the last row and before the closing line.</summary>
        public Action<TextWriter>? Trailer { get; set; }
        ...
        /// <param name="table">The columns of the rows in <paramref name="ring"/>. The frame rate log's own columns if left out.</param>
        public PerfWriter(string path, IReadOnlyList<string> headerLines, PerfRing ring, Action<string>? report, PerfTable? table = null)
        {
            this.path = path; this.headerLines = headerLines; this.ring = ring; this.report = report;
            this.table = table ?? PerfColumns.Main;
        }
        ...
        public bool Start()
        {
            try
            {
                ...
                output.WriteLine(table.HeaderLine());
                output.Flush();
            }
            catch (Exception e)
            {
        ...
        void Run()
        {
            var batch = new double[BatchRows * table.Count];
        ...
        void DrainAll(double[] batch, char[] line)
        {
            ...
            do
            {
                ...
                BeforeRows?.Invoke(output!);
                for (int i = 0; i < n; i++)
                {
                    var row = new ReadOnlySpan<double>(batch, i * table.Count, table.Count);
                    if (table.TryFormatRow(row, line, out int written)) output!.Write(line, 0, written);
                }
            } while (n == BatchRows);
