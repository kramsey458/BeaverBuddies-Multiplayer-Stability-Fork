...
tests = tests.Concat(Preview5Checks.Tests()).Concat(PerformanceChecks.Tests()).Concat(ActivityTransportChecks.Tests()).Concat(CursorPreferencesChecks.Tests()).Concat(PlayerColorsChecks.Tests()).Concat(SteamLinkChecks.Tests()).Concat(PingCadenceChecks.Tests()).Concat(NetworkStatusChecks.Tests()).Concat(PanelModelChecks.Tests()).Concat(PanelLayoutChecks.Tests()).Concat(CatchUpSpeedChecks.Tests()).Concat(SpeedBoostChecks.Tests()).Concat(HostPacingChecks.Tests()).Concat(FrameRatePacingChecks.Tests()).Concat(ModWarningChecks.Tests()).Concat(ChatChecks.Tests()).Concat(SessionEndChecks.Tests()).Concat(EntitySlotCacheChecks.Tests()).Concat(WorkshopDataChecks.Tests()).Concat(JoinWindowChecks.Tests()).Concat(DesyncCheckChecks.Tests()).Concat(DirectTcpChecks.Tests()).Concat(DesyncDialogChecks.Tests()).Concat(DeterminismRuleChecks.Tests()).Concat(SendLaneChecks.Tests()).Concat(PaceChecks.Tests()).Concat(LinkChecks.Tests()).ToArray();
