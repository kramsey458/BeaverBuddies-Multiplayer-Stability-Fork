...
- **A plainer connection panel, and chat in the cursor colors.** While the panel is expanded the dot beside the sync status is its only dot. A player's row is a name and a ping (a guest sees its own ping to the host on the host's row), your own row is bold with a dash, and the collapse button has a box around it so it is not mistaken for that dash. A chat line is drawn in the color of that player's cursor, and lines already written change when you change that color. *Confirmed in a real session: a screenshot of a host and a guest shows the rows, the boxed button and two chat lines in the players' colors. Lines changing color after you change a cursor color has not been checked.*
