...
- **Wonders run on the tick.** A Wonder's activation animation, and the Iron Teeth Earth Repopulator's whole plane launch (catapult, runway, the launcher's turn between planes), ran on each player's render frames: a player at a different frame rate, or a guest waiting for the host, deactivated the Wonder and destroyed its pilots on a different tick, and the planes got different entity IDs on each computer. In multiplayer they now advance once per tick on the tick interval, using the game's own code, and the frames only draw between ticks. Saves keep the game's keys. *Tested (the game's own animation, runway and rotation code at 10, 30 and 144 FPS); not yet played. It needs a two-player test with one player's frame rate capped before it is merged; see [the design note](BeaverBuddies/Doc/WonderTiming.md).*
