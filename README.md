...
- **Dev mode is for single player.** Most of its tools (the instant unlock with Ctrl-click, a construction site's Finish now, the debug panels) change only the computer they are used on and desync a co-op game. A notice says so when a co-op game loads with dev mode on or it is turned on. Its Ctrl keys for placing a building finished and for not recovering goods are off in co-op.
